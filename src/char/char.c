...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	default:
		ShowError("Resultado nulo para armazenamento de itens.\n");
		return 1;
	}
...
int delete_char_sql(int char_id)
{
	...
	{
		ShowError("Falha na coleta de dados para remover personagem na base de dados.\n");
...
#endif
	if (save_log)
		ShowInfo("Carregando dados de personagem! (AID:"CL_BOLD"%d"CL_RESET")\n",sd->account_id);
...
{
	ShowWarning("Sincronia perdida com o Servidor de Login.\n");
}
...
void loginif_on_ready(void)
{
	...
	if( i == ARRAYLENGTH(server) )
		ShowStatus("Aguardando dados do Servidor de Mapas.\n");
}
	...
	while(RFIFOREST(fd) >= 2) {
		...
		switch( command ) {
					...
					return 0;
			{
				...
				if( acc <= 0 ) {
					ShowError("Servidor de Login recebeu uma conta inexistente!! (AID: %d)\n", acc);
					return 0;
				}
			...
			default:
				ShowError("Pacote desconhecido (0x%04x) foi recebido do Servidor de Login!! Desconectando...\n", command);
				set_eof(fd);
				return 0;
			}
	}
...
{
	ShowStatus("O Servidor de Mapas #%d foi desconectado.\n", id);
	mapif_server_reset(id);
}
...
int parse_frommap(int fd)
{
	...
	while(RFIFOREST(fd) >= 2) {
		...
		switch(RFIFOW(fd,0)) {
			...
			case 0x2b0b:
				...
				for(i = 0; i < j; i++) {
					if( RFIFOW(fd, 4 + (i*4)) > MAX_SKILL_ID ) {
						ShowWarning("Erro skillid2dx[%d] = %d falhou, pois (%d) excede o permitido (%d).\n",RFIFOW(fd, 4 + (i*4)), RFIFOW(fd, 6 + (i*4)),RFIFOW(fd, 4 + (i*4)),MAX_SKILL_ID);
						continue;
					}
			...
			case 0x2afa: // Receiving map names list from the map-server
				...
				ShowStatus("Servidor de Mapas %d conectado! (Mapas: %d | IP: %d.%d.%d.%d | Porta: %d).\n",
							id, j, CONVIP(server[id].ip), server[id].port);
				ShowStatus("Carregamento completo do Servidor de Mapas %d.\n", id);
				...
				int x;
				if (j == 0) {
					ShowWarning("O Servidor de Mapas %d possui NENHUM mapa...\n", id);
				} else {
					...
					return 0;
			{
				...
				if( SQL->NumRows(sql_handle) > 0 ) {
					...
					if (count >= 50)
						ShowWarning("Excesso de tipos de status para (AID: %d | CID: %d). Ignorando... \n", aid, cid);
					if (count > 0) {
			...
			case 0x2afe: //set MAP user count
				...
				if (RFIFOW(fd,2) != server[id].users) {
					...
					ShowInfo("Contagem de Contas Ativas: %d (Servidor: %d)\n", server[id].users, id);
				}
				RFIFOSKIP(fd, 4);
				break;
					...
					return 0;
			{
				...
				for(i = 0; i < server[id].users; i++) {
					...
					{
						ShowNotice("Personagem (AID: %d CID: %d) marcado no Servidor de Mapas %d, mas outro %d alega possuir (AID: %d CID: %d) online!\n",
					...
					return 0;
			{
				...
				{
					ShowError("Falha no pedido para salvar personagem! (Dados Corrompidos %d != %d).\n", size-13, sizeof(struct mmo_charstatus));
					RFIFOSKIP(fd,size);
					break;
				}
					...
					character->char_id == cid))
				{
					...
					ShowError("Falha no pedido para salvar personagem: Personagem Inexistente (AID: %d CID: %d).\n", aid, cid);
	...
	//					result = 2; // 2-gm level too low
			...
			case 0x2736: // ip address update
				...
				ShowInfo("Atualizando IP do Servidor de Mapas #%d to %d.%d.%d.%d.\n", id, CONVIP(server[id].ip));
				RFIFOSKIP(fd,6);
			break;
			...
			default:
			{
				...
				ShowError("Pacote desconhecido (0x%04x) do Servidor de Mapas. Desconectando...\n", RFIFOW(fd,0));
				set_eof(fd);
				return 0;
			}
		} // switch
...
int lan_subnetcheck(uint32 ip)
{
	...
	if( i < subnet_count ) {
		ShowInfo("Checagem[Mask]: [%u.%u.%u.%u]: -> "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask));
		return subnet[i].map_ip;
	} else {
		ShowInfo("Checagem[Mask]: [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
		return 0;
	}
}
...
{// CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	ShowInfo(CL_RED"Pedido para remover personagem! "CL_GREEN"(AID: %d | CID: %d)"CL_RESET".\n", sd->account_id, char_id);
	...
	while( RFIFOREST(fd) >= 2 ) {
		...
		switch( cmd ) {
					...
					return 0;
			{
				...
				ShowInfo("Pedido de entrada! (AID: %d | ID: %d | ID2: %d)\n", account_id, login_id1, login_id2);
				...
				FIFOSD_CHECK(3);
			{
				...
				ShowInfo("Selecionando Personagem. (AID: %d | Slot: %d - %s).\n", sd->account_id, slot, char_dat.name);
				...
				if (i < 0 || !cd->last_point.map) {
					...
					if (j == ARRAYLENGTH(server)) {
						ShowInfo("Falha... Servidor de Mapas inexistente.\n");
						WFIFOHEAD(fd,3);
						WFIFOW(fd,0) = 0x81;
					...
					ShowWarning("Falha em buscar um Servidor de Mapas para '%s'! Enviando-o para cidade principal de '%s'.\n", mapindex_id2name(cd->last_point.map), mapindex_id2name(j));
					cd->last_point.map = j;
				}
				...
				{
					ShowError("Falha de Escrita (FD: %d). Servidor de Mapas #%d foi desconectado.\n", map_fd, i);
					server[i].fd = -1;
				...
				if (cmd == 0x1fb) FIFOSD_CHECK(56);
			{
				...
				if(strcasecmp(email, sd->email) && //email does not matches and
				(
			...
			default:
				ShowError("Pacote desconhecido ("CL_WHITE"0x%x"CL_RESET") foi recebido pelo IP '"CL_WHITE"%s"CL_RESET"'! Desconectando...!\n", RFIFOW(fd,0), ip2str(ipl, NULL));
				set_eof(fd);
				return 0;
			}
	}
...
int check_connect_login_server(int tid, int64 tick, int id, intptr_t data) {
	...
	ShowInfo("Tentando conectar-se ao Servidor de Login.\n");
...
int char_lan_config_read(const char *lancfgName)
{
	...
	if( subnet_count > 1 ) /* only useful if there is more than 1 */
		ShowStatus("Lendo dados sobre %d subredes.\n", subnet_count);
...
void sql_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		else if(!strcasecmp(w1,"interreg_db"))
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if(strcasecmp(w1,"timestamp_format") == 0) {
			...
			if (login_ip) {
				...
				ShowStatus("IP do Servidor de Login: %s -> %s\n", w2, ip2str(login_ip, ip_str));
			}
			...
			if (char_ip) {
				...
				ShowStatus("IP do Servidor de Personagens: %s -> %s\n", w2, ip2str(char_ip, ip_str));
			}
			...
			if( i%3 )
			{
				ShowWarning("Faltam dados para os itens de iniciantes. Ajustando..\n");
				if( i%3 == 1 )
					start_items[i-1] = 0;
				else
					start_items[i-2] = 0;
		...
		} else if (strcasecmp(w1, "char_del_level") == 0) { //disable/enable char deletion by its level condition [Lupus]
			...
			if (fame_list_size_chemist > MAX_FAME_LIST) {
				ShowWarning("Limite para personagens no ranking: %d (Classe Alquimista)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_smith > MAX_FAME_LIST) {
				ShowWarning("Limite para personagens no ranking: %d (Classe Ferreiro)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_taekwon > MAX_FAME_LIST) {
				ShowWarning("Limite para personagens no ranking: %d (Classe Taekwon)\n", MAX_FAME_LIST);
...
int do_init(int argc, char **argv) {
	...
	if (strcmp(userid, "s1")==0 && strcmp(passwd, "p1")==0) {
		ShowWarning("Usar a conta-mestre (s1/p1) pode diminuir a estabilidade do servidor.\n");
	}
	...
	if( (char_fd = make_listen_bind(bind_ip,char_port)) == -1 ) {
		ShowFatalError("Falha em atrelar na porta '"CL_WHITE"%d"CL_RESET"'\n",char_port);
		exit(EXIT_FAILURE);
	}
...
#endif
	ShowStatus("O Servidor de Personagens encontra-se "CL_GREEN"PRONTO"CL_RESET" (Ouvindo a porta %d).\n\n", char_port);
