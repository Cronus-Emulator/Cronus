...
static int auction_end_timer(int tid, int64 tick, int id, intptr_t data) {
	...
	if( (auction = (struct auction_data *)idb_get(auction_db_, id)) != NULL )
	{
		if( auction->buyer_id )
		{
			mail_sendmail(0, "Gerente do Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Parabéns, você arrematou a compra no leilão.", 0, &auction->item);
			mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
			mail_sendmail(0, "Gerente do Leilão", auction->seller_id, auction->seller_name, "Leilão", "Pagamento pelo Leilão", auction->price, NULL);
		}
		else
...
static void mapif_parse_Auction_cancel(int fd)
{
	...
	mail_sendmail(0, "Gerente do Leilão", auction->seller_id, auction->seller_name, "Leilão", "Leilão Cancelado", 0, &auction->item);
	auction_delete(auction);
...
static void mapif_parse_Auction_close(int fd)
{
	...
	mail_sendmail(0, "Gerente do Leilão", auction->seller_id, auction->seller_name, "Leilão", "Leilão Finalizado", auction->price, NULL);
	// Send Item to Buyer
	mail_sendmail(0, "Gerente do Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Vencedor do Leilão", 0, &auction->item);
	mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
	auction_delete(auction);
...
static void mapif_parse_Auction_bid(int fd)
{
	...
	{ // Send Money back to the previous Buyer
		...
		{
			mail_sendmail(0, "Gerente do Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Alguém deu um lance mais alto", auction->price, NULL);
			mapif_Auction_message(auction->buyer_id, 7); // You have failed to win the auction
		}
		else
			mail_sendmail(0, "Gerente do Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Você deu um lance mais alto", auction->price, NULL);
	}
	...
	{ // Automatic won the auction
		...
		mail_sendmail(0, "Gerente do Leilão", auction->seller_id, auction->seller_name, "Leilão", "Pagamento pelo Leilão.", auction->buynow, NULL);
