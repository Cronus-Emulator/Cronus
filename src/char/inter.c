...
void geoip_final(bool shutdown) {
	...
	if (geoip.active) {
		if (!shutdown)
			ShowStatus("GeoIP "CL_RED"desativado"CL_RESET".\n");
		geoip.active = false;
	}
}
...
static int inter_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(!strcasecmp(w1,"char_server_ip")) {
			strcpy(char_server_ip,w2);
		} else
		if(!strcasecmp(w1,"char_server_port")) {
			char_server_port = atoi(w2);
		} else
		if(!strcasecmp(w1,"char_server_id")) {
			strcpy(char_server_id,w2);
		} else
		if(!strcasecmp(w1,"char_server_pw")) {
			strcpy(char_server_pw,w2);
		} else
		if(!strcasecmp(w1,"char_server_db")) {
			strcpy(char_server_db,w2);
		} else
		if(!strcasecmp(w1,"default_codepage")) {
			strcpy(default_codepage,w2);
		}
		else if(!strcasecmp(w1,"party_share_level"))
			party_share_level = atoi(w2);
		else if(!strcasecmp(w1,"log_inter"))
			log_inter = atoi(w2);
