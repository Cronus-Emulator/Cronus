...
int parse_fromchar(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 ) {
		...
		case 0x2740: // Accinfo request forwarded by charserver on mapserver's account
			if( RFIFOREST(fd) < 22 )
				...
				if (accounts->load_num(accounts, &acc, account_id)) {
					...
					/** Caso o dado recebido pelo pacote (ID do grupo) seja maior ou igual o armazenado, a autenticação(senha)  é copiada para o buffer (32+1)
					*   Se não for o buffer é preenchido com caracteres nulo em 33 posições.
					**/
					...
					/** Caso o dado recebido pelo pacote(ID do grupo) seja maior ou igual o armazenado, a autenticação(pin) é copiada para o buffer (4+1)
					*   Se não for o buffer é preenchido com caracteres nulo em 5 posições.
					**/
					if (u_group >= acc.group_id) 
		...
		break;
		default:
			ShowError("Pacote desconhecido 0x%x recebido pelo Servidor de Personagens! Desconectando...\n", command);
			set_eof(fd);
			return 0;
		} // switch
	} // while
...
int mmo_auth(struct login_session_data* sd, bool isServer) {
	...
	if( sd->sex != 'S' && sd->account_id < START_ACCOUNT_NUM )
		ShowWarning("Conta %s tem um ID %d! ID de contas precisam ser maiores do que %d para funcionar corretamente!! \n", sd->userid, sd->account_id, START_ACCOUNT_NUM);
...

	{
		...
		{// account is already marked as online!
			...
			{// Request char servers to kick this account out. [Skotlex]
				uint8 buf[6];
				ShowNotice("Conta '%s' online. Rejeitando pedido\n", sd->userid);
				WBUFW(buf,0) = 0x2734;
...
int parse_login(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 ) {
				...
				return 0;
		{
			...
			{
				ShowStatus("Sucesso ao sincronizar com o Servidor de Personagens '%s'. (Aceito) \n", server_name);
			...
			{
				ShowNotice("Falha em sincronizar com o Servidor de Personagens '%s'. (Recusado)\n", server_name);
				WFIFOHEAD(fd,3);
				WFIFOW(fd,0) = 0x2711;
				WFIFOB(fd,2) = 3;
				WFIFOSET(fd,3);
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if(!strcasecmp(w1,"timestamp_format"))
			...
			if( msg_silent ) /* only bother if we actually have this enabled */
				ShowInfo("Omitir Mensagens: LV %d %d\n", atoi(w2));
		}
		else if( !strcasecmp(w1, "bind_ip") ) {
			login_config.login_ip = host2ip(w2);
			if( login_config.login_ip ) {
				char ip_str[16];
				ShowStatus("Servidor de Login atrelando-se ao IP : %s -> %s\n", w2, ip2str(login_config.login_ip, ip_str));
			}
		}
		...
		else if(!strcasecmp(w1, "allowed_regs")) //account flood protection system
...
int do_init(int argc, char** argv)
{
	...
	ShowStatus("O Servidor de Login encontra-se "CL_GREEN"PRONTO"CL_RESET" (Ouvindo pela porta %u).\n\n", login_config.login_port);
