...
| Special Thanks to brAthena Project for the @reloadcash idea              | 
|                                                                          |
...
struct atcmd_binding_data* get_atcommandbind_byname(const char* name) {
	...
	ARR_FIND( 0, atcommand->binding_count, i, strcasecmp(atcommand->binding[i]->command, name) == 0 );
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if (strcasecmp(w1, "import") == 0) {
			...
			if (msg_number >= 0 && msg_number < MAX_MSG) {
				...
				atcommand->msg_table[msg_number] = aMalloc((strlen(w2) + 1)*sizeof (char));
...
ACMD(item)
{
	...
	if (!strcasecmp(info->command,"itembound") && (!message || !*message || (
	...
	if(!strcasecmp(info->command,"itembound") ) {
...
ACMD(item2)
{
	...
	if (!strcasecmp(info->command,"itembound2") && (!message || !*message || (
	...
	if( !strcasecmp(info->command,"itembound2") && !(bound >= IBT_MIN && bound <= IBT_MAX) ) {
		clif->message(fd, msg_txt(298)); // Invalid bound type
		return false;
	}
	...
	if (item_id > 500) {
		...
		loop = 1;
		get_count = number;
		if( !strcasecmp(info->command,"itembound2") )
			bound = 1;
		if( !itemdb->isstackable2(item_data) ) {
			if( bound && (item_data->type == IT_PETEGG || item_data->type == IT_PETARMOR) ) {
				clif->message(fd, msg_txt(498)); // Cannot create bounded pet eggs or pet armors.
...
ACMD(monster)
{
	...
	if (strcasecmp(info->command, "monstersmall") == 0)
		size = SZ_SMALL;
	else if (strcasecmp(info->command, "monsterbig") == 0)
		size = SZ_BIG;
	else
		size = SZ_MEDIUM;
...
ACMD(killmonster) {
	...
	drop_flag = strcasecmp(info->command, "killmonster2");
...
ACMD(param) {
	...
	ARR_FIND( 0, ARRAYLENGTH(param), i, strcasecmp(info->command, param[i]) == 0 );
...
ACMD(char_ban)
{
	...
	chrif->char_ask_name(sd->status.account_id, atcmd_player_name, !strcasecmp(info->command,"charban") ? 6 : 2, year, month, day, hour, minute, second); // type: 2 - ban; 6 - charban
	clif->message(fd, msg_txt(88)); // Character name sent to char-server to ask it.
...
ACMD(char_unban)
{
	...
	// send answer to login server via char-server
	chrif->char_ask_name(sd->status.account_id, atcmd_player_name, !strcasecmp(info->command,"charunban") ? 7 : 4, 0, 0, 0, 0, 0, 0); // type: 4 - unban account; type 7 - unban character
	clif->message(fd, msg_txt(88)); // Character name sent to char-server to ask it.
...
ACMD(skillid) {
	...
	for( data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key) ) {
		...
		if (strncasecmp(key.str, message, skillen) == 0 || strncasecmp(skill->db[idx].desc, message, skillen) == 0) {
			...
			if (i == AUTOLOOTITEM_SIZE) {
				...
				for(i = 0; i < AUTOLOOTITEM_SIZE; i++)
				{
					...
					if (!(item_data = itemdb->exists(sd->state.autolootid[i]))) {
						ShowDebug("Item inexistente (%d) na lista de captura (AID: %d | CID: %d)", sd->state.autolootid[i], sd->status.account_id, sd->status.char_id);
						continue;
					}
...
ACMD(pettalk)
{
	...
	{// pet emotion processing
		...
		int i;
		ARR_FIND( 0, ARRAYLENGTH(emo), i, strcasecmp(message, emo[i]) == 0 );
...
ACMD(homlevel) {
	...
	if( (htype = homun->class2type(hd->homunculus.class_)) == HT_INVALID ) {
		ShowError("Classe de homunculo desconhecida (%d) (Jogador: %s)\n", hd->homunculus.class_,sd->status.name);
		return false;
	}
	...
	switch( htype ) {
		...
		case HT_S:
			...
			break;
		default:
			ShowError("Tipo de homunculo desconhecido: '%d'\n",htype);
			return false;
	}
...
ACMD(cash)
{
	...
	if( !strcasecmp(info->command,"cash") ) {
		if( value > 0 ) {
...
ACMD(clone) {
	...
	if (strcasecmp(info->command, "clone") == 0)
		flag = 1;
	else if (strcasecmp(info->command, "slaveclone") == 0) {
		flag = 2;
		if(pc_isdead(sd)){
...
ACMD(itemlist)
{
	...
	if( strcasecmp(info->command, "storagelist") == 0 ) {
		location = "storage";
		items = sd->status.storage.items;
		size = MAX_STORAGE;
	} else if( strcasecmp(info->command, "cartlist") == 0 ) {
		location = "cart";
		items = sd->status.cart;
		size = MAX_CART;
	} else if( strcasecmp(info->command, "itemlist") == 0 ) {
		location = "inventory";
...
ACMD(stats)
{
	...
	sprintf(job_jobname, "Classe - %s %s", pc->job_name(sd->status.class_), "(Nível %d)");
...
ACMD(addperm) {
	...
	bool add = (strcasecmp(info->command, "addperm") == 0) ? true : false;
	int i;
	...
	ARR_FIND(0, perm_size, i, strcasecmp(pcg->permissions[i].name, message) == 0);
...
ACMD(join) {
	...
	if( hChSys.local && strcasecmp(name + 1, hChSys.local_name) == 0 ) {
	...
	} else if( hChSys.ally && sd->status.guild_id && strcasecmp(name + 1, hChSys.ally_name) == 0 ) {
...
ACMD(channel) {
	...
	if( strcasecmp(subcmd,"create") == 0 && ( hChSys.allow_user_channel_creation || pc_has_permission(sd, PC_PERM_HCHSYS_ADMIN) ) ) {
		if( sub1[0] != '#' ) {
		...
		if( strcasecmp(sub1 + 1,hChSys.local_name) == 0 || strcasecmp(sub1 + 1,hChSys.ally_name) == 0 || strdb_exists(clif->channel_db, sub1 + 1) ) {
		...
		if( sub1[0] != '\0' && strcasecmp(sub1,"colors") == 0 ) {
			char mout[40];
	...
	} else if ( strcasecmp(subcmd,"setcolor") == 0 ) {
		...
		for( k = 0; k < hChSys.colors_count; k++ ) {
			if( strcasecmp(sub2,hChSys.colors_name[k]) == 0 )
				break;
		}
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"leave") == 0 ) {
		...
		for(k = 0; k < sd->channel_count; k++) {
			if( strcasecmp(sub1+1,sd->channels[k]->name) == 0 )
				break;
		}
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"bindto") == 0 ) {
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"unbind") == 0 ) {
		...
		sd->gcbind = NULL;
	} else if ( strcasecmp(subcmd,"ban") == 0 ) {
		struct map_session_data *pl_sd = NULL;
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"unban") == 0 ) {
		struct map_session_data *pl_sd = NULL;
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"unbanall") == 0 ) {
		if( sub1[0] != '#' ) {
			clif->message(fd, msg_txt(1405));// Channel name must start with a '#'
			return false;
		}
		...
		clif->message(fd, atcmd_output);
	} else if ( strcasecmp(subcmd,"banlist") == 0 ) {
	...
	} else if ( strcasecmp(subcmd,"setopt") == 0 ) {
		...
		for( k = 1; k < 3; k++ ) {
			if( strcasecmp(sub2,opt_str[k]) == 0 )
				break;
		}
...
ACMD(costume){
	...
	for( k = 0; k < len; k++ ) {
		if( strcasecmp(message,names[k]) == 0 )
			break;
	}
	if( k == len ) {
...
void atcommand_config_read(const char* config_filename) {
	...
	if (aliases != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			if ( !( commandinfo = atcommand_exists(commandname) ) ) {
				ShowConfigWarning(command, "Falha em determinar apelido para comando: %s", commandname);
				continue;
			}
			...
			for (j = 0; j < alias_count; ++j) {
				...
				if (alias != NULL) {
					...
					if (strdb_exists(atcommand->alias_db, alias)) {
						ShowConfigWarning(command, "Apelido %s duplicado.", alias);
						continue;
					}
	...
	if (nolog != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			commandname = config_setting_name(command);
			if ( !( commandinfo = atcommand_exists(commandname) ) ) {
				ShowConfigWarning(command, "Falha em desativar log: Comando inexistente %s", commandname);
				continue;
			}
	...
	if (help != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			commandname = config_setting_name(command);
			if ( !( commandinfo = atcommand_exists(commandname) ) )
				ShowConfigWarning(command, "Comando %s inexistente", commandname);
			else {
				if( commandinfo->help == NULL ) {
					const char *str = libconfig->setting_get_string(command);
					size_t len = strlen(str);
					commandinfo->help = aMalloc(len);
	...
	ShowStatus("Finalizada leitura de '"CL_WHITE"%d"CL_RESET"' apelidos dos comandos em '"CL_WHITE"%s"CL_RESET"'.\n", num_aliases, config_filename);
...
void atcommand_db_load_groups(GroupSettings **groups, config_setting_t **commands_, size_t sz)
{
	...
	for (atcmd = dbi_first(iter); dbi_exists(iter); atcmd = dbi_next(iter)) {
		...
		for (i = 0; i < sz; i++) {
			...
			if (group == NULL) {
				ShowError("Grupo Nulo\n");
				continue;
			}

			idx = pcg->get_idx(group);
			if (idx < 0 || idx >= sz) {
				ShowError("Valor (%d) ultrapassou os limites permitidos [0,%d]\n", idx, sz - 1);
				continue;
			}
