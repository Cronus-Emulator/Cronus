...
int unit_walktoxy_timer(int tid, int64 tick, int id, intptr_t data) {
	...
	struct unit_data        *ud = NULL;
	struct mercenary_data   *mrd = NULL;
	...
	if(ud->walktimer != tid){
		ShowError("Temporizador de unidade encontra-se desproporcional (%d != %d).\n",ud->walktimer,tid);
		return 0;
	}
	ud->walktimer = INVALID_TIMER;
	if (!bl->prev) return 0; // Stop moved because it is missing from the block_list
...
int unit_run(struct block_list *bl) {
	...
	nullcheck(bl); // If only sc->data get NULL, bl is dereferenced in another function
...
int unit_attack(struct block_list *src,int target_id,int continuous) {
	struct block_list *target = map->id2bl(target_id);
	struct unit_data  *ud = NULL;
...
int unit_skillcastcancel(struct block_list *bl,int type)
{
	...
	if( ret < 0 )
		ShowError("Falha em remover o temporizador (%d) (Habilidade %d - %s)\n",ret,skill_id,skill->get_name(skill_id));
