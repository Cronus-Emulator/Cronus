...
static int duel_showinfo_sub(struct map_session_data* sd, va_list va)
{
	...
	clif_disp_onlyself(ssd, output, (uint32_t)strlen(output));
	return 1;
}
...
void duel_invite(const unsigned int did, struct map_session_data* sd, struct map_session_data* target_sd) {
	...
	clif->disp_message(&sd->bl, output, (uint32_t)strlen(output), DUEL_WOS);
	...
	clif->broadcast((struct block_list *)target_sd, output, (uint32_t)strlen(output)+1, BC_BLUE, SELF);
}
